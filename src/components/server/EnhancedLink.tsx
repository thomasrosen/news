...
export async function EnhancedLink({ children, href }: { children: React.ReactNode; href: string }) {
    ...
    if (linkData) {
      return <HoverCard openDelay={100} closeDelay={200}>
        <HoverCardTrigger asChild>
          <a
            href={href}
            className={cn('hover:opacity-90 transition-all decoration-primary/60 hover:decoration-primary underline underline-offset-2 decoration-2 group')}
          >
            {
...
export async function EnhancedLinkButton({ children, href, variant = 'default' }: { children: React.ReactNode; href: string, variant?: 'default' | 'outline' | 'ghost' | 'secondary' } ) {
  const boxed_styles = 'my-3 no-underline flex! justify-start font-bold!'

  if (!['default', 'outline', 'ghost'].includes(variant)) {
    variant = 'secondary'
  }
  if (variant === 'default') {
    variant = 'secondary'
    ...
    if (linkData) {
      return <HoverCard openDelay={100} closeDelay={200}>
        <HoverCardTrigger asChild>
          ...
          <a
            href={href}
            className={cn('hover:opacity-90 transition-all decoration-primary/60 hover:decoration-primary underline underline-offset-2 decoration-2 group', boxed_styles)}
          >
            {
              linkData.metadata.coverphoto
                ? <span className="inline-block align-text-bottom me-1 relative w-4 h-4 shrink-0">
  ...
  return <Button variant={variant} size="lg" asChild><a
    href={href}
    className={cn('hover:opacity-90 transition-all decoration-primary/60 hover:decoration-primary underline underline-offset-2 decoration-2 group', boxed_styles)}
    target={href.startsWith('http') ? '_blank' : undefined}
    rel={href.startsWith('http') ? 'noopener noreferrer nofollow' : undefined}
  >
    {children || href}
    {href.startsWith('http') ? <Icon name="arrow_outward" size="sm" className="transition-opacity align-middle opacity-0 group-hover:opacity-30" /> : null}
  </a></Button>
}
